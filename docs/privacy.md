...
- Manual searches and city details additionally use a bounded six-hour memory cache and shared request budget. Destination photos load only in the explorer from image URLs supplied by Nomads.com, including its resizeapi.com image service; those hosts see normal image requests and the connection's IP address. Source and meetup links open Nomads.com in the browser when clicked.
